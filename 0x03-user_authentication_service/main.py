#!/usr/bin/env python3
"""Module contains integration tests that validate the expected
status code responses and payloads for app.py"""
...
def profile_unlogged() -> None:
    """Tests for retrieving profile info while unlogged"""
...
def log_in(email: str, password: str) -> str:
    ...
    res = requsts.post(url, data=body)
    assert res.status_code == 200
    assert res.json() == {"email": email, "message": "logged in"}
    return res.cookies.get('session_id')
